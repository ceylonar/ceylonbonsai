...
import { addArtworkItem, updateArtworkItem, deleteArtworkItem } from '@/lib/artwork';
import { useToast } from '@/hooks/use-toast';

const emptyItem: Omit<ArtworkItem, 'id'> = {
  title: '',
  description: '',
...
interface ArtworkAdminProps {
  items: ArtworkItem[];
  setItems: React.Dispatch<React.SetStateAction<ArtworkItem[]>>;
}

export default function ArtworkAdmin({ items, setItems }: ArtworkAdminProps) {
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [currentItem, setCurrentItem] = useState<Partial<ArtworkItem> | null>(null);
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this artwork item?')) {
      try {
        await deleteArtworkItem(id);
        setItems(items.filter((item) => item.id !== id));
        toast({ title: 'Item Deleted', description: 'The artwork item has been removed.' });
      } catch (error) {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (currentItem.id) {
        ...
        await updateArtworkItem(id, dataToUpdate as Omit<ArtworkItem, 'id'>);
        setItems(items.map(item => item.id === id ? { ...item, ...dataToUpdate, id } as ArtworkItem : item));
        toast({ title: 'Item Updated', description: 'The artwork item has been saved.' });
      } else {
        // Add new item
        const newItem = await addArtworkItem(currentItem as Omit<ArtworkItem, 'id'>);
        setItems([...items, newItem]);
        toast({ title: 'Item Added', description: 'The new artwork item has been added.' });
      }
      setIsFormOpen(false);
      setCurrentItem(null);
    } catch (error) {
