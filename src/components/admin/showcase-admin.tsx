...
import { addShowcaseItem, updateShowcaseItem, deleteShowcaseItem } from '@/lib/showcase';
...
interface ShowcaseAdminProps {
  items: ShowcaseItem[];
  setItems: React.Dispatch<React.SetStateAction<ShowcaseItem[]>>;
}

export default function ShowcaseAdmin({ items, setItems }: ShowcaseAdminProps) {
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [currentItem, setCurrentItem] = useState<Partial<ShowcaseItem> | null>(null);
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this item?')) {
      try {
        ...
        toast({ title: 'Item Deleted', description: 'The showcase item has been removed.' });
      } catch (error) {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (currentItem.id) {
        ...
        await updateShowcaseItem(id, dataToUpdate as Omit<ShowcaseItem, 'id'>);
        setItems(items.map(item => item.id === id ? { ...item, ...dataToUpdate, id } as ShowcaseItem : item));
        toast({ title: 'Item Updated', description: 'The showcase item has been saved.' });
      } else {
        // Add new item
        const newItem = await addShowcaseItem(currentItem as Omit<ShowcaseItem, 'id'>);
        setItems([...items, newItem]);
        toast({ title: 'Item Added', description: 'The new showcase item is now on the homepage.' });
      }
      setIsFormOpen(false);
      setCurrentItem(null);
    } catch (error) {
    ...
    <div>
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>{currentItem && currentItem.id ? 'Edit' : 'Add'} Showcase Item</DialogTitle>
          </DialogHeader>
          {currentItem && (
